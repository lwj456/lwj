...
def main():
    ...
    # we evaluate if only a portion of data are watermarked audio, what happens?
    wm_ratio_lst = [1.0, 0.80, 0.60, 0.40, 0.20]

    for wm_ratio in wm_ratio_lst:
        for tgt_name in speaker_names_lst:
            ...
            config = ExpSpeakerAdaptYourTTSCfg(
                ...
                tgt_audio_dic_lst=get_existing_wm_samples(tgt_name, out_dir.joinpath("ExpEmbedWatermark"), wm_ratio=wm_ratio),
            ...
            folder_name = "ExpSpeakerAdaptYourTTS"

            if wm_ratio < 1.0:
                folder_name = folder_name + f"_wm_ratio_{wm_ratio:.2f}"      # specify the ratio in the folder name

            exp_dir = out_dir.joinpath(folder_name)
