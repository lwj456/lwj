...
def train(run_id: str, syn_dir: Path, models_dir: Path, save_every: int, backup_every: int, force_restart: bool,
          hparams, model_ckpt_path, on_train_step_start):
    ...
    for i, session in enumerate(hparams.tts_schedule):
        ...
        for epoch in range(1, epochs+1):
            for i, (texts, mels, embeds, idx) in enumerate(data_loader, 1):
                ...
                if on_train_step_start is not None:
                    if on_train_step_start(cur_iter, model) is False:
                        return
